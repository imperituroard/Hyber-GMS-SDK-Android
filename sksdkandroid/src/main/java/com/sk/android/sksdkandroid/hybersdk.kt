...
import com.sk.android.sksdkandroid.add.HyberParsing
import com.sk.android.sksdkandroid.core.*
import com.sk.android.sksdkandroid.core.Initialization
import com.sk.android.sksdkandroid.core.HyberApi
import com.sk.android.sksdkandroid.core.HyberDataApi
...
class HyberSDK(user_msisdn: String="unknown", user_password: String="unknown", context: Context) {
    ...
    private var answer_not_registered: HyberFunAnswerGeneral = HyberFunAnswerGeneral(704, "Failed", "Registration data not found", "Not registered")
    private var answer_not_known: HyberFunAnswerGeneral = HyberFunAnswerGeneral(710, "Failed", "Unknown error", "unknown")
    ...
    //1
    fun hyber_register_new(X_Hyber_Client_API_Key: String, X_Hyber_App_Fingerprint: String): HyberFunAnswerRegister {
        try {
            if (init_hyber.paramsglobal.registrationstatus == true) {
                return answ.hyber_register_new_register_exists2(init_hyber.paramsglobal,context)
            } else {
                val answ_sk: HyberDataApi2 = apihyber.hyber_device_register(
    ...
    fun hyber_clear_current_device(): HyberFunAnswerGeneral {
        try {
            if (init_hyber.paramsglobal.registrationstatus == true) {
                ...
                Log.d(TAG, "sk_answer : ${hyber_answer.toString()}");

                if (hyber_answer.code == 200) {
                    Log.d(TAG, "start clear data");
                    init_hyber.clearData()
                    return answ.general_answer("200", "{\"device\":\"${init_hyber.paramsglobal.deviceId}\"}", "Success")
                } else {
                    if  (hyber_answer.code == 401){
                        try{ init_hyber.clearData()} catch (ee:Exception){}
                    }
                    return answ.general_answer(hyber_answer.code.toString(), "{\"body\":\"unknown\"}", "Some problem")
                }
            } else {
    ...
    fun hyber_get_message_history(period_in_seconds: Int): HyberFunAnswerGeneral {
        try {
            if (init_hyber.paramsglobal.registrationstatus == true) {
                ...
                return answ.general_answer(mess_hist_hyber.code.toString(), mess_hist_hyber.body, "Success")
            } else {
                return answer_not_registered
            }
        } catch (e: Exception) {
    ...
    fun hyber_get_device_all_from_hyber(): HyberFunAnswerGeneral {
        try {
            if (init_hyber.paramsglobal.registrationstatus == true) {
                ...
                Log.d(TAG, "device_all_sk : $device_all_hyber");
                if  (device_all_hyber.code == 401){
                    try{ init_hyber.clearData()} catch (ee:Exception){}
                }
                return answ.general_answer(device_all_hyber.code.toString(), device_all_hyber.body, "Success")
            } else {
                return answer_not_registered
            }
        } catch (e: Exception) {
    ...
    //6
    fun hyber_send_message_callback(message_id: String, message_text: String): HyberFunAnswerGeneral {
        try {
            if (init_hyber.paramsglobal.registrationstatus == true) {
                val respp: HyberDataApi = apihyber.sk_message_callback(
                    message_id,
                    message_text,
                    X_Hyber_Session_Id,
                    init_hyber.paramsglobal.hyber_registration_token
    ...
    fun hyber_message_delivery_report(message_id: String): HyberFunAnswerGeneral {
        try {
            if (init_hyber.paramsglobal.registrationstatus == true) {
                val respp1: HyberDataApi = apihyber.sk_message_dr(
                    message_id,
                    X_Hyber_Session_Id,
                    init_hyber.paramsglobal.hyber_registration_token
                )
    ...
    fun hyber_clear_all_device(): HyberFunAnswerGeneral {
        try {
            if (init_hyber.paramsglobal.registrationstatus == true) {
                ...
                Log.d(TAG, "sk_answer : $hyber_answer");
